...
function Circle(x, y, dx, dy, radius, colorString) {
  ...
  this.updatePosition = function () {
    if (
      this.x + this.radius > bounceBoundaryRight ||
      this.x - this.radius < bounceBoundaryLeft
    ) {
      this.dx = -this.dx;
    }
    if (
      this.y + this.radius > bounceBoundaryBottom ||
      this.y - this.radius < bounceBoundaryTop
    ) {
      this.dy = -this.dy;
    }
...
function init() {
  ...
  for (let i = 0; i < numberOfDots; i++) {
    ...
    // velocity
    let dx = (Math.random() - 0.5) * 2 * 0.1;
    let dy = (Math.random() - 0.5) * 2 * 0.1;
