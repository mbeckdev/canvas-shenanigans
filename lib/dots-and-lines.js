  ...
  if (circleArrayIndex > 0) {
    ...
    for (let i = 1; i < circleArrayIndex + 1; i++) {
      ...
      if (
        ...
        // colorString = 'hsla(' + hue + '270, 50%, 50%, ' + colorAlpha + ')';
        // draw line between two dots
        drawALine(x1, y1, x2, y2, colorString);
        // drawALine(x1, y1, x2, y2, 'hsla(270, 50%, 50%, 0.5)');
      } else if (lengthBetweenDots < minDistanceToDraw) {
        colorAlpha = 1;
        colorString = 'hsla(' + endHue + ', 100%, 50%, ' + colorAlpha + ')';
        drawALine(x1, y1, x2, y2, colorString);
      }
    }
  } else {
...
function chooseLineColor(
  ...
  let hueRange = hueMax - hueMin;
  let hue = colorAlpha * hueRange + hueMin;
  colorString = 'hsla(' + hue + '270, 100%, 50%, ' + colorAlpha + ')';
  return colorString;
}
...
function init() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    let dx = (Math.random() - 0.5) * 2 * 0.25; // velocity
    let dy = (Math.random() - 0.5) * 2 * 0.25; // velocity
